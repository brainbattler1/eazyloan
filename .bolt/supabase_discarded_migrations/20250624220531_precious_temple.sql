/*
  ...
  2. Functions
    ...
    - `create_system_message` - Create system message
  ...
  4. Real-time
    - Triggers for automatic loan notifications
    - Proper indexing for performance
*/

-- Drop existing notification tables and functions to start fresh
DROP TABLE IF EXISTS notifications CASCADE;
DROP TABLE IF EXISTS system_messages CASCADE;

-- Drop notification-specific functions only
...
-- Helper function to check if user is admin (create only if doesn't exist)
...
-- Helper function to check if user is super admin (create only if doesn't exist)
